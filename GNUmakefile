...
## min rust version is rustc 1.72.0 ## therefore we install via rustup
	...
	bash -c "[ '$(shell uname -s)' == 'Linux' ] && echo 'is Linux' && $(shell echo which apt-get) &&  apt-get install autoconf clang cmake expat gettext libexpat1-dev libssl-dev python3 python-is-python3 zlib* --fix-missing && $(shell echo which cargo) || echo 'add apt-get install sequence';"
